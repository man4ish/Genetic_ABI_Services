...
\definecolor{headcolwithoutgrr}{rgb}{.118, .565, 1.00}
\definecolor{rowcolwithoutgrr}{rgb}{.714, .855, .957}
...
if (runtype == 1)
{
    ...
    if (nrow(data2) != 0 ){
          for (rs in 1:nrow(data2)) {
                 ...
                 cat(paste(data2[rs,1],"& ", data2[rs,4] ,"&",data2[rs,5] ,"&\\parbox[t]{5cm}{", data2[rs,9] ,"~\\\\[-5pt]}\\\\",sep=""),"\n")
                 cat("\\hline","\n")
            }
    }
     ...
     if (nrow(data3) != 0 ){
          for (rs in 1:nrow(data3)) {
                 ...
                 cat(paste(data3[rs,1],"& ", data3[rs,4] ,"&",data3[rs,5] ,"&\\parbox[t]{5cm}{", data3[rs,8] ,"~\\\\[-5pt]}\\\\",sep=""),"\n")
                 cat("\\hline","\n")
            }
    }
    cat("\\end{longtable}}","\n")
...
if (xmlSize(diseases) !=0)
{
   ...
   {       
         ...
         {             
             ...
             tsumflag<-0
             for (c in 1:xmlSize(rowtable)) { if(nchar(xmlValue(rowtable[c][["GenoEffect"]][["Genotypes"]][["Genotype"]][["TraitInfo"]][["TraitSummary"]])) > 1) {tsumflag<-1} }

             if(tsumflag==1)
             {                
                 cat(paste("\\textbf{",name,"}",sep=""),"\n")
                 #cat("~\\\\[-10pt]","\n")                         
                 cat("{\\renewcommand{\\arraystretch}{1.8}\\begin{longtable}{|p{1.8cm}|M{3.9cm}|M{.7cm}|M{3.5cm}|}","\n")
                 cat("\\hline","\n")
                 ...
                 {      
                   ...
                   page_num<-""
                   pagekey<-paste(rsid,"-",disid,sep="")
                   if (runtype == 1){                       
                       page_num<-indexmap[[pagekey]]
                   }
                   cat("\\rowcolor{rowcol}","\n")
                   if(nchar(TraitSummary) > 1) {
                      cat(paste(rsid,"/ ",gene,"& ","\\parbox[t]{3.8cm}{",TraitSummary,"~\\\\[-5pt]}& ",page_num, "&",TraitRecommendations, "\\\\",sep=""),"\n")
                      cat("\\hline","\n")
                   }
                 }
                 cat("\\end{longtable}}","\n")
    ...
    cat("\\begin{center}","\n")
    cat("\\textbf{\\large ",TraitResultInterpretationTitle,"}\n")
    cat("\\end{center}","\n")
    cat("~\\\\","\n")
    cat("~\\\\","\n")
...
#         } 
         ...
         if (lang == "ru") { cat(Text,"\n"); } 
           ...
           {                              
               ...
               references<-diseases[i][["Disease"]]["References"]
               
               NumberOfTraitMarkers<-xmlValue(top[["Titles"]][["NumberOfTraitMarkers"]])
               NumberOfTraitMarkers<-sub("\\$\\{disease_name\\}",disname,NumberOfTraitMarkers) 
               ...
               for (k in 1:xmlSize(genoeffects))
               {
                   ...
                   risk<-xmlGetAttr(genoeffects[k][["GenoEffect"]],"risk")
                   ...
                   if(risk !=0)
                   {
                      ...
                      {  
                         ...
                         {                       
                            ...
                            if(GRRFlag == "0.00")
                            {
                                  cat("{\\renewcommand{\\arraystretch}{1.8}\\begin{longtable}{|p{2.5cm}|M{2.45cm}|M{3.0cm}|M{2.0cm}|}","\n")
                            ...
                            refmap<-numeric(0)
                            for (c in 1:length(refid[[1]]))
                            {
                               #cat(paste(refs),"\n")
                               refs<-refid[[1]][c]
                             
                               indexvector<-strsplit(refs,",")
                               indexvector[[1]]<-sort(indexvector[[1]])
                               #cat(paste(refs,"*****",indexvector),"\n")
                               idtext<-numeric(0)
                               idtext<-paste(idtext,which(pmid==indexvector[[1]][1]),sep="")
                               len<-length(indexvector[[1]])
                               #cat(paste(idtext,indexvector[[1]],pmid),"\n")
                               #push(refmap,idtext)          
                               if (len > 1){                             
                                   for (cc in 2:len) { 
                             ...
                             for (p in 1:nrow) {
                                ...
                                if(GRRFlag == "0.00") {
                                            cat(paste(" & ",RelatedCondition," &[",refmap[p],"]\\\\",sep=""),"\n")
                                            #cat("\\cline{3-5}","\n")
                                } else {
                                            cat(paste(" & ",RelatedCondition," &",GRRValue," &[",refmap[p],"]\\\\",sep=""),"\n")
                                            cat("\\cline{3-5}","\n")
                                }                     
                             }
                             ...
                             if(GRRFlag=="0.00") {
                                ...
                                cat("\\rowcolor{rowcolwithgrr}","\n")
                                cat(paste("\\multicolumn{4}{|p{11.25cm}|}{\\itshape",Text,sep=""),"}\\\\","\n")    #need to add italicisse and repalce with disease tbale for Keliod foramtion.
                              } else {
                                cat("\\rowcolor{rowcolwithoutgrr}","\n")
                                #cat(paste("\\multicolumn{5}{|p{11.25cm}|}{",Text,sep=""),"}\\\\","\n")
                                cat(paste("\\multicolumn{5}{|p{11.25cm}|}{\\itshape{",Text,"}",sep=""),"}\\\\","\n")
                              }                
                              cat("\\hline","\n")               
                              cat("\\end{longtable}}","\n")
                              cat("~\\\\[-10pt]","\n")
       ...
       if(traitflag == 'N' || disid == "OST_01_06")
       {
         ...
         if (length(rowtable) != 0) {
             for (nc in 1:length(rowtable)) {
              ...
              {      
                   ...
                   #if ( (traitflag == 'N' && RelativeRisk > 1.00 ))
                   {
                       cat("\\rowcolor{rowcol}","\n")
                       rc<-rc+1
                       rsid<-xmlValue(rowtable[c][["GenoEffect"]][["Genotypes"]][["Genotype"]][["SNP"]]) 
         ...
         if (length(ref) != 0)
         {
             if(length(pmid) > 0 ) {cat(paste("\\footnotesize \\textcolor{black}{\\textbf{",References_title,"}}",sep=""),"\n")}
...
if (runtype == 1)
{
    ...
    for (rs in 1:rsize) {
         ...
         Recommendation3<-xmlValue(diseases[rs][["Disease"]][["G3Contents"]][["Recommendations"]][["Recommendation3"]])
         disname<-xmlGetAttr(top[["Summary"]][["Results"]][rs][["Result"]],"Name")
         #cat(DiseaseID,disname,file="Test.txt",Recommendation1,Recommendation2,Recommendation3,append=TRUE,"\n")         
    ...
    #write.table(data2, file = "temp.txt")
    if (lang=="ru") {
   ...
   {       
         ...
         {             
             ...
             if(tsumflag==1)
             {
                 cat(paste("\\textbf{",name,"}",sep=""),"\n")
                 cat("~\\\\[-10pt]","\n")
                 #cat(paste("\\textbf{\\textcolor{black}{\\large",name,"}",sep=""),"\n")
                 ...
                 {      
                      ...
                      page_num<-""
                      pagekey<-paste(rsid,"-",disid,sep="")
                      if (runtype == 1){ page_num<-indexmap[[pagekey]] }
                      cat("\\rowcolor{rowcol}","\n")
                      if(nchar(TraitSummary) > 1) {
                         cat(paste(rsid,"/ ",gene,"& ","\\parbox[t]{3.8cm}{",TraitSummary,"~\\\\[-5pt]}& ",page_num, "&",TraitRecommendations, "\\\\",sep=""),"\n")
                         cat("\\hline","\n")
...
if (runtype == 1)
{
    ...
    if (nrow(sorteddata_disease) > 3) {
        if (lang == "in" && (ChipType == "CRD")) {             
            ...
            typicalsortedrowwidth<-6/nrow(sorteddata_disease)
        }   
    } else {
        if (ChipType == "CRD") {
            typicalsortedrowwidth<-8.5/nrow(sorteddata)  
        } else {   
            typicalsortedrowwidth<-7.1/nrow(sorteddata_disease)           
        }     
    }
