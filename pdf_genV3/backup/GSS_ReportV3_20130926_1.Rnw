...
GenGRRPlot <- function(IR,Gender,GRR, AvGRR, Interval, name, DiseaseID, age ) {              #new function added for plot generation
    ...
    if(ChipType == "PDC"|| ChipType == "PDI") {
      ...
      if(DiseaseID == "ONC_01_07" || DiseaseID== "ONC_01_27" ) { diffy<-(max(y)-min(y))/2 } else {diffy<-(max(y)-min(y))/10}
...
if (nrow(sorteddata) != 0 )
{
   ...
   if (nrow(data3) != 0 ) {
      cat("\\rowcolor[RGB]{255,255,51}")   
      cat(paste("\\parbox[t][",totalwidth,"cm]{4.5cm}{\\textcolor{black}{",DoctorNote3,"}}\\\\",sep=""),"\n")
      cat("\\hline","\n")
   }
   ...
   if (nrow(sorteddata_trait) != 0 )
   {
        for (rs in 1:nrow(sorteddata_trait)) {
             if (sorteddata_trait[rs,2] != "0" ) {    
                 ...
                 cat(paste("\\parbox[t][1cm]{4.5cm}{\\textcolor{black}{","\\cellcolor{rowcol}",count_risk_markers,"}}\\\\",sep=""),"\n")
                 cat("\\hline","\n")
             }   
        }
   }
    ...
    {       
         ...
         if (traitflag == 'N') {  
             ...
             if (disid== "PDI_01_10" || disid == "ONC_01_26") { 
                 GLR<-round((GRR*(1-exp(-IR*maxage/10000)))*1000,3) 
                 AvGLR<-round((AvGRR*(1-exp(-IR*maxage/10000)))*1000,3)
             } else {
             ...
             #Note<-xmlValue(top[["Summary"]][["TypicalAndDecreasedRiskTable"]][["Note1"]])
             Note<-sub("<=","\\$\\\\leq\\$",Note) 
            
             
             cat("\\begin{minipage}{0.36\\linewidth}","\n")
             #cat("{\\renewcommand{\\arraystretch}{1.8}\\begin{longtable}{|p{1.6cm}|p{1.6cm}|p{1.8cm}|}","\n")
             ...
             cat("\\hline","\n")
             cat("\\rowcolor{rowcol}","\n") 
             #cat(paste("\\parbox[t][1.5cm]{1.6cm}{\\textbf{",TblGenetic_row1_title,"}} & \\textbf{\\textcolor{black}{", data[5,i],"}} & \\textbf{\\textcolor{black}{", GLR,"\\%}}\\\\",sep=""),"\n")
             ...
             #cat(paste("\\parbox[t][1.5cm]{1.6cm}{\\textbf{",TblGenetic_row2_title,"}}  & \\textbf{\\textcolor{black}{", round(AvGRR,2),"}} & \\textbf{\\textcolor{black}{",AvGLR,"\\%}}\\\\",sep=""),"\n") 
             cat("\\hline","\n")
             cat("\\end{longtable}}","\n")
             cat("\\end{minipage}","\n")            
             ...
             #cat(paste("\\textbf{\\textcolor{white}{","Genetic Relative Risk (GRR) and ",TblGenetic_col3_title,"}}\\\\",sep=""),"\n")
...
if(increasedrisk_count != 0 ) 
{  
   cat("\\begin{minipage}{0.55\\linewidth}","\n")
  cat("{\\renewcommand{\\arraystretch}{1.8}\\begin{longtable}{|p{3.4cm}|M{1.1cm}|M{.87cm}|}","\n")
   cat("\\hline","\n")

   cat(paste("\\rowcolor{textcol} \\textbf{ \\textcolor{white}{",TblIncreasedRisk_col1_title,"}} &\\textbf{\\textcolor{white}{",tblTypicalAndDecreasedRisk_col2_title,"}}  &\\textbf{\\textcolor{white}{",TblIncreasedRisk_col6_title,"}} \\\\",sep=""),"\n")
   cat("\\endfirsthead","\n")
   cat(paste("\\rowcolor{textcol} \\textbf{ \\textcolor{white}{",TblIncreasedRisk_col1_title,"}} &\\textbf{\\textcolor{white}{",tblTypicalAndDecreasedRisk_col2_title,"}}  &\\textbf{\\textcolor{white}{",TblIncreasedRisk_col6_title,"}} \\\\",sep=""),"\n")
    cat("\\endhead","\n")
    cat("\\hline","\n")
  ...
  if(nrow(data1) ==0 || nrow(data2)==0)
   ...
   if(nrow(data3) != 0 )
    ...
    if (nrow(sorteddata_trait) != 0 )
    {
        for (rs in 1:nrow(sorteddata_trait)) {
             if (sorteddata_trait[rs,2] != "0" ) {
             cat("\\rowcolor{rowcol}","\n") 
             cat(paste("\\parbox[t][1cm]{2.8cm}{\\raggedright ",sorteddata_trait[rs,1],"} & ", "N/A" ,"&",sorteddata_trait[rs,5]   ,"\\\\",sep=""),"\n")      
             cat("\\hline","\n")
             }
        }
    }  
   ...
   cat(paste("\\rowcolor{textcol} \\textbf{ \\textcolor{white}{",TblIncreasedRisk_col3_title,"}} \\\\[1.16cm]",sep=""),"\n")
   cat("\\endhead","\n")
   totalwidth<-rowwidth1*nrow(data1) + 0.2694*(nrow(data1)-1)
   cat("\\hline","\n")
   cat("\\rowcolor[RGB]{243,189,185}","\n")
   
   DoctorNote1<-xmlValue(top[["Summary"]][["IncreasedRiskTable"]][["DoctorNote1"]])
   DoctorNote1<-sub(">=","\\$\\\\geq\\$",DoctorNote1)
   DoctorNote2<-xmlValue(top[["Summary"]][["IncreasedRiskTable"]][["DoctorNote2"]])   
   DoctorNote3<-xmlValue(top[["Summary"]][["IncreasedRiskTable"]][["DoctorNote3"]])
   
   if (nrow(data1) != 0 ) { 
      cat(paste("\\parbox[t][",totalwidth,"cm]{4.5cm}{\\textcolor{black}{",DoctorNote1,"}}\\\\",sep=""),"\n")
      cat("\\hline","\n")
   }
   
   totalwidth<-rowwidth2*nrow(data2) + 0.2694*(nrow(data2)-1)
   cat("\\rowcolor[RGB]{251,204,157} ","\n")
   if (nrow(data2) != 0 ) { 
       cat(paste("\\parbox[t][",totalwidth,"cm]{4.5cm}{\\textcolor{black}{",DoctorNote2,"}}\\\\",sep=""),"\n") 
       cat("\\hline","\n")
   }
   ...
   {
   if (nrow(data3) != 0 ) {
      totalwidth<-rowwidth3*nrow(data3) + 0.2694*(nrow(data3)-1)
      cat("\\rowcolor[RGB]{255,255,51} ")
      cat(paste("\\parbox[t][",totalwidth,"cm]{4.5cm}{\\textcolor{black}{",DoctorNote3,"}}\\\\",sep=""),"\n")
      cat("\\hline","\n")
   }
  ...
  #if ( (nrow(data1) != 0 && nrow(data2) != 0)  && (nrow(data3) != 0 ||nrow(sorteddata_trait) != 0 ) && (increasedflag == 1))
  
  if(increasedflag != 1 ) #temporary fix for ONC
  {
           cat("\\begin{minipage}{0.55\\linewidth}","\n")
  cat("{\\renewcommand{\\arraystretch}{1.8}\\begin{longtable}{|p{3.4cm}|M{1.1cm}|M{.87cm}|}","\n")
   cat("\\hline","\n")
    ...
    if (nrow(sorteddata) != 0 )
    {
        ...
        for (rs in 1:disease_count) {
              ...
              if (sorteddata[rs,2] == "0" ) {
                 traits<-1
                 cat("\\rowcolor{rowcol}","\n") 
                 cat(paste("\\parbox[t][",typicalsortedrowwidth,"cm]{3.4cm}{\\raggedright ",sorteddata[rs,1],"} & ", "N/A" ,"&",sorteddata[rs,5]   ,"\\\\",sep=""),"\n") 
                 cat("\\hline","\n")
             } else {             
                cat(paste("\\parbox[t][",typicalsortedrowwidth,"cm]{3.4cm}{\\raggedright ",sorteddata[rs,1],"}","& ", sorteddata[rs,4], "& ", sorteddata[rs,5],"\\\\",sep=""),"\n")
                cat("\\hline","\n") 
