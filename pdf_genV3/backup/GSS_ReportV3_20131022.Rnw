...
#maxrisk<-read.table("/home/manish/dataset/MaxRiskfile.log",sep="\t")
#maxmap=list()
#for (ls in 1:nrow(maxrisk))
#{
#     maxmap[[maxrisk[ls,1]]]=maxrisk[ls,2]
#     cat(paste(maxrisk[ls,1],maxrisk[ls,2],sep="\t"),file="tata",append=TRUE,"\n")
#}
...
GenGRRPlot <- function(IR,Gender,GRR, AvGRR, Interval, name, DiseaseID, age ) {              #new function added for plot generation
    ...
    #cat(paste(segy,maxGLR,sep="\t"),file=paste("Test_File2.txt"),append=TRUE,"\n")
    if(DiseaseID=="CRD_01_02" || DiseaseID=="CRD_01_04" ||DiseaseID=="CRD_01_09" ||DiseaseID=="CRD_01_11" ||DiseaseID=="CRD_01_12" ||DiseaseID=="CRD_01_14" ||DiseaseID=="MTB_01_01"||DiseaseID=="MTB_01_12"||DiseaseID=="MTB_01_04"||DiseaseID=="MTB_01_05"||DiseaseID=="MTB_01_09" ||DiseaseID=="OST_01_07"||DiseaseID=="OST_01_08"||DiseaseID=="OST_01_09" ||DiseaseID=="PDC_01_02"||DiseaseID=="PDC_01_04"||DiseaseID=="PDC_01_07"||DiseaseID=="PDC_01_08"||DiseaseID=="PDC_01_09")
    { 
    ...
    if(ChipType == "PDC"|| ChipType == "PDI") {
      ...
      text(x=age+2,y=0,age,cex=1.8,pos=3)
      text(x=max(x)*3/100,y=segy,round(segy,2),cex=1.8,pos=3)
    }else {
      plot(x, y, type="l", lwd=3, col=col1,ylim=c(0, max(y,z)*1.2), cex.lab=2,cex.names=1.8,cex.axis=1.8, xlim=c(0,max(x)+5), xlab="Age (Years)",ylab="", main=paste(name),xaxs="i",yaxs="i", yaxt="n",cex.main =3, las =0,xpd=TRUE)
