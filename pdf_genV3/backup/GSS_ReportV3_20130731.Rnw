...
#for (ilen in 1:(length(names(riskfile))-1)) 
#{
#     cat(paste(names(riskfile)[ilen],mean(riskfile[[names(riskfile)[ilen]]]),sep="\t"),file=fn,append=TRUE,"\n")            
#}

IRmap=list()
IRmap[["Atrial Fibrillation"]]=254
IRmap[["Hypertension"]]=552.5
IRmap[["Abdominal Aortic Aneurysm"]]=1.45
IRmap[["Brain Aneurysm"]]=1.12
IRmap[["Chronic Obstructive Pulmonary Disease"]]=42.55
IRmap[["Coronary Heart Disease"]]=805
IRmap[["Deep Vein Thrombosis and Pulmonary Embolism"]]=13.575
IRmap[["Peripheral Arterial Disease"]]=19
IRmap[["Stroke"]]=210
IRmap[["Sudden Cardiac Arrest"]]=8.6325
IRmap[["Hypertriglyceridemia"]]=8.6325
IRmap[["Obesity"]]=8.6325
IRmap[["Familial Hypercholesterolemia"]]=8.6325
IRmap[["Heart Attack"]]=8.6325
############################################################################################################################
...
GenGRRPlot <- function(IR,Gender,GRR, AvGRR, Interval, name, DiseaseID, age ) {              #new function added for plot generation
    ...
    pdf(paste("./tmp/",DiseaseID,".pdf",sep=""),width=16,height=8)     #this path and/or name need to changed
    layout(matrix(c(1,2), nrow = 1), widths = c(0.74, 0.26))
    par(mar=c(6,6,3,.5))
    col1<-numeric(0)
    ...
    legend("center",legend=c("You","General Population"), lwd=c(2,2), col=c(col1,"black"), cex=1.8, lty = c(1, 1), pch = c(23, 23),merge = TRUE, bg = "white",pt.bg=c(col1,"black"))
    #cat(paste(IR,Gender,GRR,AvGRR,Interval, name,DiseaseID,age,segy, sep="\t"),file=filename,append=TRUE,"\n")
    dev.off()
}
...
cat(paste("\\fancyhead[OR]{\\begin{mdframed}[linecolor=white,roundcorner=0pt,backgroundcolor=white,innerleftmargin=0,innertopmargin=0,innerbottommargin=0]\\parbox[t]{1.035\\textwidth}{\\raggedleft","\\includegraphics[width=0.18\\linewidth]{",logo,"}}","\\parbox[t]{0.333\\textwidth}{\\centering Page}\\parbox[t]{0.333\\textwidth}{\\raggedright 3} \\end{mdframed}}",sep=""),"\n")
cat("\\definecolor{textcol}{rgb}{.118, .565, 1.00}","\n")
cat("\\fancyfoot[C]{\\begin{mdframed}[roundcorner=0pt,leftmargin=0, rightmargin=0, linecolor=white,outerlinewidth=.1, innerleftmargin=0,innertopmargin=0,innerbottommargin=0,innerrightmargin=0,everyline = true, splittopskip=.6cm, splitbottomskip=.3cm]","\n")
...
if (runtype == 1)
{
    ...
    newdata <- data.frame(col1=column1, col2=column2, col3=column3, col4=column4,col5=column5,col6=column6)
    sorteddata <- newdata[order(newdata$col4,decreasing = TRUE), ]

    newdata_trait <- data.frame(col1=column1_trait, col2=column2_trait, col3=column3_trait, col4=column4_trait,col5=column5_trait,col6=column6_trait)
    sorteddata_trait <- newdata_trait[order(newdata_trait$col4,decreasing = FALSE), ]

    cat("{\\renewcommand{\\arraystretch}{1.8}\\begin{longtable}{|P{2.7cm}|M{1.0cm}|P{2.4cm}|M{1.0cm}|M{1.3cm}|P{0.8cm}|}","\n")
    cat("\\hline","\n")

    cat(paste("\\rowcolor{textcol} \\textbf{ \\textcolor{white}{",TblIncreasedRisk_col1_title,"}} &\\textbf{ \\textcolor{white}{",TblIncreasedRisk_col2_title,"}} &\\textbf{ \\textcolor{white}{",TblIncreasedRisk_col3_title,"}}  &\\textbf{ \\textcolor{white}{",TblIncreasedRisk_col4_title,"}} &\\textbf{ \\textcolor{white}{",TblIncreasedRisk_col5_title,"}} &\\textbf{ \\textcolor{white}{",TblIncreasedRisk_col6_title,"}} \\\\",sep=""),"\n")
    cat("\\endfirsthead","\n")
    ...
    if (nrow(sorteddata) != 0 )
    {
        ...
        for (rs in 1:nrow(sorteddata)) {
             if (as.numeric(as.character(sorteddata[rs,4])) > 2.00) { 
                cat("\\rowcolor[RGB]{243,189,185}","\n")   #{254,179,164},{253,162,137}                
                #if (R_Type == '0') { 
                #    R_Type<-'1' 
                #    cat("\\rowcolor[RGB]{243,189,185}","\n")   #{254,179,164},{253,162,137} 
                #}
                ...
                #}
                cat("\\hline","\n")
                cat("\\rowcolor[RGB]{251,204,157} ")   # {249,177,105
             }
              
             cat(paste(sorteddata[rs,1]," & ",sorteddata[rs,4]))

             ###########################Manish Change######################################
            
             AvGRR<-mean(as.numeric(riskfile[[sorteddata[rs,6]]]))
             GRR<-as.numeric(as.character(sorteddata[rs,4]))
             disease_name<-sorteddata[rs,1] 
             #IR<-IRmap[[sorteddata[rs,1]]]                     #due to radnom value retrivement from IRmap, IR values have been hardcoded (these values will be extracted from xml later)
             IR<-numeric(0)
             if(disease_name == "Abdominal Aortic Aneurysm") 
             {
                   IR<-1.45
             } else if(disease_name == "Brain Aneurysm")
             {
                   IR<-1.12
             } else if(disease_name == "Atrial Fibrillation")
             {
                   IR<-254
             } else if(disease_name == "Chronic Obstructive Pulmonary Disease")
             {
                   IR<-42.55
             } else if(disease_name == "Coronary Heart Disease")
             {
                   IR<-805
             } else if(disease_name == "Deep Vein Thrombosis and Pulmonary Embolism")
             {
                   IR<-13.575
             } else if(disease_name == "Familial Hypercholesterolemia")
             {
                   IR<-12                                                #dummy value
             } else if(disease_name == "Heart Attack")
             {
                   IR<-12                                                #dummy value
             } else if(disease_name == "Hypertension")
             {
                   IR<-552.5
             } else if(disease_name == "Hypertriglyceridemia")
             {
                   IR<-12                                                 #dummy value
             } else if(disease_name == "Obesity")
             {
                   IR<-12                                                 #dummy value
             } else if(disease_name == "Peripheral Arterial Disease")
             {
                   IR<-19
             } else if(disease_name == "Stroke")
             {
                   IR<-210
             }
             else if(disease_name == "Sudden Cardiac Arrest")
             {
                   IR<-8.6325
             }

             GLR<-round((GRR*(1-exp(-IR*70/10000)))*100,2)
             AvGLR<-round((AvGRR*(1-exp(-IR*70/10000)))*100,2)
            
             #######################################################################

             #if (R_Type == '1' ){                                       
             #    cat(paste(" & \\multirow{1}{*}{\\textcolor{black} \\tiny Note1} &", GLR,"\\% & ", AvGLR ,"\\% & ",sorteddata[rs,5]," \\\\",sep=""),"\n")
             #    R_Type<-'9' 
             #} else if (R_Type == '2' ){                 
             #    cat(paste(" & \\multirow{3}{*}{\\textcolor{black} \\tiny Note2} &", GLR, "\\% & ",AvGLR ,"\\% & ",sorteddata[rs,5]," \\\\\\cline{1-2} \\cline{4-6}",sep=""),"\n")        
             #    R_Type<-'8' 
             #} else {
             #    cat(paste(" & &", GLR,"\\% &", AvGLR,"\\% & ",sorteddata[rs,5]," \\\\\\cline{1-2} \\cline{4-6}",sep=""),"\n")
             #}  
                       
             if (AvGLR != 1) {              
                 cat(paste(" & &", GLR,"\\% &", AvGLR,"\\% & ",sorteddata[rs,5]," \\\\",sep=""),"\n")    
             } else {
                 cat(paste(" & &", GLR,"\\% &", AvGLR,".00\\% & ",sorteddata[rs,5]," \\\\",sep=""),"\n")
             }
        }
        cat("\\hline","\n") 
    }
    ...
    if (nrow(sorteddata_trait) != 0 )
    {
        for (rs in 1:nrow(sorteddata_trait)) {
             cat("\\rowcolor{rowcol}","\n") 
             cat(paste(sorteddata_trait[rs,1],sep=""),"\n")
             cat (paste("& N/A",sep=""),"\n")

             if (sorteddata_trait[rs,3] != "0" ) { 
                 count_risk_markers<-sub("\\$\\{count\\}",sorteddata_trait[rs,3],count_risk_markers_Tmp)
                 cat(paste("& \\multicolumn{2}{c|}{",count_risk_markers,"}",sep=""),"\n") 
             }
             else {
                 cat(paste("& \\multicolumn{2}{c|}{",no_risk_markers,"}",sep=""),"\n") 
             }

             #cat(paste("& ", " ",sep=""),"\n")
             cat(paste("& N/A",sep=""),"\n")
             cat(paste("& ", sorteddata_trait[rs,5]," \\\\",sep=""),"\n")
...
################################### Typical and Decreased Genetic Risk Summary ###################
...
if (runtype == 1)
{
    ...
    for (rs in 1:rsize) {
         ...
         if(length(riskfile[[DiseaseID]]) !=0)
         {
                  ...
                  {
                        GenGRRPlot(IR,"N",GRR,AvGRR,5,disname,DiseaseID,31)
                  }
          } 
    ...
    cat("{\\renewcommand{\\arraystretch}{1.8}\\begin{longtable}{|P{2.7cm}|M{1.0cm}|P{2.4cm}|M{1.0cm}|M{1.3cm}|P{0.8cm}|}","\n")
    cat("\\hline","\n")
    cat(paste("\\rowcolor{textcol} \\textbf{ \\textcolor{white}{",tblTypicalAndDecreasedRisk_col1_title,"}} &\\textbf{ \\textcolor{white}{",tblTypicalAndDecreasedRisk_col2_title,"}} &\\textbf{ \\textcolor{white}{",tblTypicalAndDecreasedRisk_col3_title,"}}  &\\textbf{ \\textcolor{white}{",tblTypicalAndDecreasedRisk_col4_title,"}} &\\textbf{ \\textcolor{white}{",tblTypicalAndDecreasedRisk_col5_title,"}} &\\textbf{ \\textcolor{white}{",tblTypicalAndDecreasedRisk_col6_title,"}} \\\\",sep=""),"\n")
    cat("\\endfirsthead","\n")
    ...
    if (nrow(sorteddata) != 0 )
    {
        Note<-xmlValue(top[["Summary"]][["TypicalAndDecreasedRiskTable"]][["Note"]])
        for (rs in 1:nrow(sorteddata)) {

             ####################################Change Manish###########################################                        
             disease_id<-numeric(0)
             disease_id<-sorteddata[rs,6]
             xxx<-riskfile[[disease_id]]
             AvGRR<-numeric(0)
             GRR<-numeric(0)
             disease_name<-numeric(0)
             IR<-numeric(0)
             #AvGRR<-mean(as.numeric(as.character(avmap[[disease_id]])))                       #due to radnom value retrivement from IRmap and avmpa, IR values have been hardcoded (these values will be extracted from xml later)
             ...
             #AvGRR<-mean(as.numeric(riskfile[[sorteddata[rs,6]]])) //checked later

             GLR<-(GRR*(1-exp(-IR*70/10000)))*100
             GLR<-round(GLR,2)
             AvGLR<-(AvGRR*(1-exp(-IR*70/10000)))*100
             AvGLR<-round(AvGLR,2)
             #cat(paste(sorteddata[rs,6],GRR,AvGRR,IR,AvGLR,GLR),"\n")
             #######################################################################

             cat("\\rowcolor[RGB]{255,255,0}","\n")
             ...
             #cat(paste(sorteddata[rs,1],"& ", sorteddata[rs,4],"& - & - & - & ", sorteddata[rs,5],sep=""),"\n")  
             cat("\\hline","\n")
        }
    }
...
#GeneralLifeStyle_pageno<-cat("\\thepage","\n")
#\newwrite\miscfile
#\openout\miscfile=\jobname.misc
...
Content2<-xmlValue(top[["GeneralLifeStyle"]][["Content2"]])
...
Content1<-xmlValue(top[["RiskScoresDefinition"]][["Content1"]][["Text"]])
cat(paste("{",Content1,"}",sep=""),"\n")

Formula1<-xmlValue(top[["RiskScoresDefinition"]][["Content1"]][["Formula1"]])
Formula2_Tmp<-xmlValue(top[["RiskScoresDefinition"]][["Content1"]][["Formula2"]])
Formula2_Tmp<-sub("\\$\\{color_red_start\\}","\\\\textcolor\\{red\\}\\{",Formula2_Tmp)
Formula2<-sub("\\$\\{color_red_stop\\}","\\}",Formula2_Tmp)
Formula3<-xmlValue(top[["RiskScoresDefinition"]][["Content1"]][["Formula3"]])

cat("{\\begin{longtable}{P{1.4cm}P{9.6cm}}","\n")
cat(paste("  & ",Formula1,"\\\\",sep=""),"\n")
cat(paste("\\normalsize ",Formula2," & \\begin{flushleft} \\linethickness{1mm} \\line(1,0){260} \\end{flushleft} \\\\",sep=""),"\n")
cat(paste("  & ",Formula3," \\\\",sep=""),"\n")
cat("\\end{longtable}}","\n")

Content2<-xmlValue(top[["RiskScoresDefinition"]][["Content2"]][["Text"]])
...
SubTitle3<-xmlValue(top[["RiskScoresDefinition"]][["Content3"]][["SubTitle"]])
cat(paste("{ \\normalsize ",SubTitle3,"}",sep=""),"\n")
cat("~\\\\","\n")
cat("~\\\\","\n")
cat("~\\\\","\n")
Content3<-xmlValue(top[["RiskScoresDefinition"]][["Content3"]][["Text"]])
...
SubTitle4<-xmlValue(top[["RiskScoresDefinition"]][["Content4"]][["SubTitle"]])
cat(paste("{ \\normalsize ",SubTitle4,"}",sep=""),"\n")
cat("~\\\\","\n")
cat("~\\\\","\n")
cat("~\\\\","\n")
Content4<-xmlValue(top[["RiskScoresDefinition"]][["Content4"]][["Text"]])
...
SubTitle5<-xmlValue(top[["RiskScoresDefinition"]][["Content5"]][["SubTitle"]])
cat(paste("{ \\normalsize ",SubTitle5,"}",sep=""),"\n")
cat("~\\\\","\n")
cat("~\\\\","\n")
cat("~\\\\","\n")
Content5<-xmlValue(top[["RiskScoresDefinition"]][["Content5"]][["Text"]])
...
Content6<-xmlValue(top[["RiskScoresDefinition"]][["Content6"]][["Text"]])
cat(paste("{",Content6,"}",sep=""),"\n")
cat("~\\\\","\n")
cat("~\\\\","\n")
cat("~\\\\","\n")
Formula_Tmp<-xmlValue(top[["RiskScoresDefinition"]][["Content6"]][["Formula"]])
Formula_Tmp<-gsub("\\$\\{color_red_start\\}","\\\\textcolor\\{red\\}\\{",Formula_Tmp)
Formula<-gsub("\\$\\{color_red_stop\\}","\\}",Formula_Tmp)
cat(paste("{ \\normalsize ",Formula,"}",sep=""),"\n")
cat("~\\\\","\n")
cat("~\\\\","\n")
cat("~\\\\","\n")


Content7<-xmlValue(top[["RiskScoresDefinition"]][["Content7"]][["Text"]])
...
if (xmlSize(diseases) !=0)
{
    ...
    miscellaneousfile<-read.csv("GSS_ReportV3.misc",sep="=",header=F)
...
################################### Genetic Table ###################
         ...
         if (traitflag == 'N') {  
             ...
             GLR<-round((GRR*(1-exp(-IR*70/10000)))*100,2)
             AvGLR<-round((AvGRR*(1-exp(-IR*70/10000)))*100,2)
             #cat(paste(AvGRR,GRR,AvGLR,GLR,sep=")))))"),"\n")

             #############################################################################

             cat("\\scriptsize ","\n")
             cat("{\\renewcommand{\\arraystretch}{1.5}\\begin{longtable}{|M{5.2cm}|M{3.2cm}|M{2.0cm}|}","\n")
             cat(paste("\\rowcolor{textcol} & \\textbf{ \\textcolor{white}{",TblGenetic_col1_title,"}} & \\textbf{ \\textcolor{white}{",TblGenetic_col2_title,"}} \\\\",sep=""),"\n")
             cat("\\hline","\n")
             cat(paste("\\rowcolor{rowcol} \\textbf{\\textcolor{black}{",TblGenetic_row1_title,"}}  & \\textbf{\\textcolor{black}{", round(AvGRR,2),"}} & \\textbf{\\textcolor{black}{",data[5,i],"}} \\\\",sep=""),"\n")
             cat("\\hline","\n")
             if (AvGLR != 1) {
                 cat(paste("\\rowcolor{rowcol} \\textbf{ \\textcolor{black}{",TblGenetic_row2_title,"}} & \\textbf{\\textcolor{black}{", AvGLR,"\\%}} & \\textbf{\\textcolor{black}{", GLR, "\\%}}\\\\",sep=""),"\n")
             } else {
cat(paste("\\rowcolor{rowcol} \\textbf{ \\textcolor{black}{",TblGenetic_row2_title,"}} & \\textbf{\\textcolor{black}{", AvGLR,".00\\%}} & \\textbf{\\textcolor{black}{", GLR, "\\%}}\\\\",sep=""),"\n")
             } 
             cat("\\hline","\n")
             cat("\\end{longtable}}","\n")
             ...
             if (file.exists(ImageFile)) {
                 cat("\\begin{center}","\n")
                 cat(paste("\\includegraphics[trim=0cm 0cm 0cm 0cm, clip=true,width=0.9\\linewidth]{",ImageFile,"}",sep=""),"\n")
                 cat("\\end{center}","\n")
            } else {
...
cat(paste("\\fancyhead[OR]{\\begin{mdframed}[linecolor=white,roundcorner=0pt,backgroundcolor=white,innerleftmargin=0,innertopmargin=0,innerbottommargin=0]\\parbox[t]{1.0\\textwidth}{\\raggedleft \\scriptsize \\textcolor{red} {",TearOffInstruction,"}}","\\parbox[t]{0.333\\textwidth}{\\centering Page}\\parbox[t]{0.333\\textwidth}{\\raggedright 3} \\end{mdframed}}",sep=""),"\n")
cat(paste("\\fancyhead[EL]{\\begin{mdframed}[linecolor=white,roundcorner=0pt,backgroundcolor=white,innerleftmargin=0,innertopmargin=0,innerbottommargin=0]\\parbox[t]{1.0\\textwidth}{\\raggedleft \\scriptsize \\textcolor{red} {",TearOffInstruction,"}}","\\parbox[t]{0.333\\textwidth}{\\centering Page}\\parbox[t]{0.333\\textwidth}{\\raggedright 3} \\end{mdframed}}",sep=""),"\n")
...
if (runtype == 1)
{
    ...
    cat("{\\renewcommand{\\arraystretch}{1.8}\\begin{longtable}{|P{2.7cm}|M{1.4cm}|P{1.6cm}|M{1.4cm}|M{1.3cm}|P{0.8cm}|}","\n")
    cat("\\hline","\n")

    TblIncreasedRisk_col2_title<-sub("Your","Patient's",TblIncreasedRisk_col2_title)
    TblIncreasedRisk_col3_title<-sub("/"," /",TblIncreasedRisk_col3_title)
    TblIncreasedRisk_col4_title<-sub("Your","Patient's",TblIncreasedRisk_col4_title)
    ...
    if (nrow(sorteddata) != 0 )
    {
        ...
        for (rs in 1:nrow(sorteddata)) {
             ...
             ############### change Manish  #############################
             AvGRR<-mean(as.numeric(riskfile[[sorteddata[rs,6]]]))   #need to get sorted diseease id
             IR<-numeric(0)  ## due to random value generation may be because of memoery leak, IR values have been hard coded below
             disease_name<-sorteddata[rs,1]
             if(disease_name == "Abdominal Aortic Aneurysm")
             {
                   IR<-1.45
             ...
             GLR<-round((GRR*(1-exp(-IR*70/10000)))*100,2)
             AvGLR<-round((AvGRR*(1-exp(-IR*70/10000)))*100,2)
             #cat(paste("*****",sorteddata[rs,1],GRR,AvGRR,IR,AvGLR,GLR),"\n") 
             ###################################################
...
if (runtype == 1)
{
    ...
    tblTypicalAndDecreasedRisk_col2_title<-sub("Your","Patient's",tblTypicalAndDecreasedRisk_col2_title)
    tblTypicalAndDecreasedRisk_col3_title<-sub("/"," /",tblTypicalAndDecreasedRisk_col3_title)
    tblTypicalAndDecreasedRisk_col4_title<-sub("Your","Patient's",tblTypicalAndDecreasedRisk_col4_title)
    ...
    if (nrow(sorteddata) != 0 )
    {
        ...
        for (rs in 1:nrow(sorteddata)) {
             ...
             disease_name<-numeric(0)
             IR<-numeric(0)
             #AvGRR<-mean(as.numeric(as.character(avmap[[disease_id]])))  # due to random value generation may be because of memoery leak, AvGRR and IR values have been hard coded below
             GRR<-as.numeric(as.character(sorteddata[rs,4]))
             disease_name<-sorteddata[rs,1]
