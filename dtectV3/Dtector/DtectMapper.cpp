    ...
    for (map<string, set<string> >::const_iterator val_assay_cit = val_assay_map.begin(); val_assay_cit != val_assay_map.end(); ++val_assay_cit)
    {
        ...
        for (set<size_t>::const_iterator row_it = assay_row_it->second.begin(); row_it != assay_row_it->second.end(); ++row_it)
        {
            ...
            if (this->is_valid_nucleotide(risk_allele))
            {
                ...
                if (relative_risk == 0.00) { relative_risk = 1.00; } //syhwah 2013-03-13

                Incidence = double(atof(all_res[i]["Incidence"].c_str())); //syhwah 2013-04-02              
            }

            int risk =  int(atof(all_res[i]["Risk"].c_str()));


            //this->gss_logger->log(true, "disdrugid=%s\trisk=%d\trelativerisk=%f\n",all_res[i]["DisDrugID"].c_str(),risk,relative_risk);

            if (strcmp(all_res[i]["Type"].c_str(),"Disease")==0) //syhwah 2013-10-28 - Only applies to Diseases
            {                  
                if (all_res[i]["Trait"].c_str()[0] == 'N') {  // syhwah 2013-11-12 do not apply to trait
                    risk = ((int)((relative_risk+0.005)*100) > 100)? -1:0; // syhwah 2013-10-29 changed as per request.
                } 
            }
    ...
    for (sim_patient_it = sim_patient_map.begin(); sim_patient_it != sim_patient_map.end(); ++sim_patient_it)
    {
        ...
        while (sim_patient_it->second.get_record(assay_id, vf_call) == true)
        {
            ...
            for (set<size_t>::const_iterator it = assay_row_it->second.begin(); it != assay_row_it->second.end(); ++it)
            {
                ...
                int risk =  int(atof(all_res[a]["Risk"].c_str()));
  
                if (strcmp(all_res[a]["Type"].c_str(),"Disease")==0) 
                { 
                    if (all_res[a]["Trait"].c_str()[0] == 'N') {  // syhwah 2013-11-12 do not apply to trait
                        risk = ((int)((relative_risk+0.005)*100) > 100)? -1:0; // syhwah 2013-10-29 changed as per request.
                    }
                }              
        ...
        const string& _lang_code)
{
    ...
    string key_patient_name, key_patient_id, key_patient_add, key_sample_id, 
           key_patient_gender, key_patient_dob, cover_TextID, cover_title,
           cover_assertion, key_primary_sample_type, key_sample_collection_date, 
           key_sample_receipt_date, key_report_date, key_reference_no, key_doctor_name, 
           key_doctor_add, label_your_drug_profile;
    ...
    if (strcmp(_lang_code.c_str(),"en") == 0) {
        cover_assertion = ""; 
    } else {
        ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "cover_assertion", cover_assertion);
    } 
    assert(ret == true);
    ...
    time_t ttime; (void) time(&ttime); struct tm *g_ptm = localtime(&ttime);

    char acPatientYearOfBirth[5]; 
    strncpy(acPatientYearOfBirth,this->dob.str().c_str(),4);

    int PatientYearOfBirth = atoi(acPatientYearOfBirth);
    int CurrentYear = 2000+g_ptm->tm_year-100;

    char PatientAge[5];
    sprintf(PatientAge,"%d",CurrentYear-PatientYearOfBirth);
    ...
    string summ_incr_risk_title, summ_incr_risk_col1, summ_incr_risk_col2, summ_incr_risk_col3, summ_incr_risk_col4, summ_incr_risk_col5, summ_incr_risk_col6,
           summ_incr_risk_note1, summ_incr_risk_note2, summ_incr_risk_note3, summ_incr_risk_note4, summ_incr_risk_dr_note1, summ_incr_risk_dr_note2, 
           summ_incr_risk_dr_note3, summ_incr_risk_dr_title;
    ...
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "summ_increased_risk_note1", summ_incr_risk_note1);
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "summ_increased_risk_note2", summ_incr_risk_note2);
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "summ_increased_risk_note3", summ_incr_risk_note3);
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "summ_increased_risk_note4", summ_incr_risk_note4);
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "summ_increased_risk_dr_note1", summ_incr_risk_dr_note1);
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "summ_increased_risk_dr_note2", summ_incr_risk_dr_note2);
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "summ_increased_risk_dr_note3", summ_incr_risk_dr_note3);
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "TITLE", "summ_increased_risk_dr_title", summ_incr_risk_dr_title);
    assert(ret == true);

    xml_ret = gss_xml_writer.insert_summ_increased_risk(
            summ_incr_risk_title.c_str(), summ_incr_risk_col1.c_str(), summ_incr_risk_col2.c_str(), summ_incr_risk_col3.c_str(), 
            summ_incr_risk_col4.c_str(), summ_incr_risk_col5.c_str(), summ_incr_risk_col6.c_str(),summ_incr_risk_note1.c_str(), 
            summ_incr_risk_note2.c_str(), summ_incr_risk_note3.c_str(), summ_incr_risk_note4.c_str() , summ_incr_risk_dr_note1.c_str(), 
            summ_incr_risk_dr_note2.c_str(), summ_incr_risk_dr_note3.c_str(),summ_incr_risk_dr_title.c_str());

    assert(xml_ret == true);
   
    //-----------------------------------------------------------------
    
    string summ_non_incr_risk_title, summ_non_incr_risk_col1, summ_non_incr_risk_col2, summ_non_incr_risk_col3, summ_non_incr_risk_col4, 
           summ_non_incr_risk_col5, summ_non_incr_risk_col6, summ_non_incr_risk_caption, summ_non_incr_risk_note;
    ...
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "summ_non_increased_risk_note", summ_non_incr_risk_note);
    assert(ret == true);
    ...
    xml_ret = gss_xml_writer.insert_summ_non_increased_risk(
            ...
            summ_non_incr_risk_col6.c_str(), summ_non_incr_risk_caption.c_str(),summ_non_incr_risk_note.c_str());
    assert(xml_ret == true);    
    ...
    string patient_riskscores_subtitle1, patient_riskscores_contents1, patient_riskscores_formula1_1, patient_riskscores_formula1_2, patient_riskscores_formula1_3,
           patient_riskscores_contents2, patient_riskscores_subtitle3, patient_riskscores_contents3, patient_riskscores_formula3, patient_riskscores_contents4; 
     
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "patient_riskscores_subtitle1", patient_riskscores_subtitle1);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "patient_riskscores_contents1", patient_riskscores_contents1);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "patient_riskscores_formula1_1", patient_riskscores_formula1_1);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "patient_riskscores_formula1_2", patient_riskscores_formula1_2);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "patient_riskscores_formula1_3", patient_riskscores_formula1_3);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "patient_riskscores_contents2", patient_riskscores_contents2);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "patient_riskscores_subtitle3", patient_riskscores_subtitle3);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "patient_riskscores_contents3", patient_riskscores_contents3);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "patient_riskscores_formula3", patient_riskscores_formula3);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "patient_riskscores_contents4", patient_riskscores_contents4);
    assert(ret == true);


    string doctor_riskscores_subtitle1,doctor_riskscores_contents1, doctor_riskscores_formula1_1, doctor_riskscores_formula1_2, doctor_riskscores_formula1_3, doctor_riskscores_contents2, 
           doctor_riskscores_subtitle3, doctor_riskscores_contents3, doctor_riskscores_subtitle4, doctor_riskscores_contents4, doctor_riskscores_subtitle5, doctor_riskscores_contents5, 
           doctor_riskscores_subtitle6, doctor_riskscores_contents6, doctor_riskscores_formula6, doctor_riskscores_contents7;

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_subtitle1", doctor_riskscores_subtitle1);
    assert(ret == true);
   
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_contents1", doctor_riskscores_contents1);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_formula1_1", doctor_riskscores_formula1_1);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_formula1_2", doctor_riskscores_formula1_2);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_formula1_3", doctor_riskscores_formula1_3);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_contents2", doctor_riskscores_contents2);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_subtitle3", doctor_riskscores_subtitle3);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_contents3", doctor_riskscores_contents3);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_subtitle4", doctor_riskscores_subtitle4);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_contents4", doctor_riskscores_contents4);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_subtitle5", doctor_riskscores_subtitle5);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_contents5", doctor_riskscores_contents5);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_subtitle6", doctor_riskscores_subtitle6);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_contents6", doctor_riskscores_contents6);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_formula6", doctor_riskscores_formula6);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_riskscores_contents7", doctor_riskscores_contents7);
    assert(ret == true);
                              
    xml_ret = gss_xml_writer.insert_riskscores_def(riskscores_title.c_str(), 
                                                   patient_riskscores_subtitle1.c_str(),patient_riskscores_contents1.c_str(), patient_riskscores_formula1_1.c_str(),
                                                   patient_riskscores_formula1_2.c_str(), patient_riskscores_formula1_3.c_str(), patient_riskscores_contents2.c_str(), 
                                                   patient_riskscores_subtitle3.c_str(), patient_riskscores_contents3.c_str(), patient_riskscores_formula3.c_str(), 
                                                   patient_riskscores_contents4.c_str(),
                                                   doctor_riskscores_subtitle1.c_str(),doctor_riskscores_contents1.c_str(), doctor_riskscores_formula1_1.c_str(), 
                                                   doctor_riskscores_formula1_2.c_str(), doctor_riskscores_formula1_3.c_str(), doctor_riskscores_contents2.c_str(), doctor_riskscores_subtitle3.c_str(), 
                                                   doctor_riskscores_contents3.c_str(), doctor_riskscores_subtitle4.c_str(), doctor_riskscores_contents4.c_str(), doctor_riskscores_subtitle5.c_str(), 
                                                   doctor_riskscores_contents5.c_str(), doctor_riskscores_subtitle6.c_str(), doctor_riskscores_contents6.c_str(), doctor_riskscores_formula6.c_str(), 
                                                   doctor_riskscores_contents7.c_str());
    assert(xml_ret == true);
    ...
    string genetic_col1, genetic_col2, genetic_col3, genetic_row1, genetic_row2, genetic_note;
    ...
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "genetic_col3", genetic_col3);
    assert(ret == true);
    ...
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "genetic_note", genetic_note);
    assert(ret == true);

    xml_ret = gss_xml_writer.insert_genetic_tbl_titles(genetic_col1.c_str(), genetic_col2.c_str(), genetic_col3.c_str(),genetic_row1.c_str(), genetic_row2.c_str(),
                                                       genetic_note.c_str());
    ...
    string writeup, label_GLR, label_age, label_you, label_general_population, label_current_status;

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "graph_writeup", writeup);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "graph_GLR", label_GLR);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "graph_age", label_age);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "graph_you", label_you);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "graph_general_population", label_general_population);
    assert(ret == true);

    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "graph_current_status", label_current_status);
    assert(ret == true);

    xml_ret = gss_xml_writer.insert_graph_writeup(writeup.c_str(), label_GLR.c_str(), label_age.c_str(), label_you.c_str(), label_general_population.c_str(), label_current_status.c_str());
    ...
    string count_risk_markers, no_risk_markers, num_of_risk_markers, num_of_pharmacogenetic_markers, ref_url, no_citation, doc_sheet,tear_off_instruction, doc_summary;
    ...
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "doctor_summary", doc_summary);
    assert(ret == true);
    ...
    string type, dis_drug_id, dis_drug_name, dis_drug_id_symptoms;
    double total_risk;
...
/*
        ...
        assert(ret == true);
*/
      dis_drug_id_symptoms = dis_drug_id;

      if ((sex != 'B') && (sex != this->gender.c_str()[0])){
           char gender_text_id[32] = {0};
           sprintf(gender_text_id, "%s_%s", dis_drug_id.c_str(), this->gender.c_str());
           dis_drug_id = gender_text_id;            
      } 

       //this->gss_logger->log(true, "Sex->%c\t gender->%s\t dis_drug_id->%s\n", sex,this->gender.c_str(),dis_drug_id.c_str());
    //-----------------------------------------------------------------
        ...
        {
             xml_ret = this->process_G3_content(gss_xml_writer, _lang_code, dis_drug_id_symptoms, "Symptoms");
             assert(xml_ret == true);
...
//this->gss_logger->log(true, " ", pdf_gen_command);
//this->gss_logger->log(true, ">%s<", pdf_gen_command);

    // generate PDF using XML
    int pdf_ret = -1;
    pdf_ret = system(pdf_gen_command);

    if (pdf_ret == 0 || pdf_ret == 256)
        return true;

    this->gss_logger->log(true, "%s\n",strerror(pdf_ret));
    return false;
}
