    ...
    const char* _open_array_id)
{
    ...
    if (assay_cnt != unique_assay_cnt){
        faulty_cnt = unique_assay_cnt - assay_cnt;
        unique_assay_cnt -= faulty_cnt;       
    }  
    ...
    ret = this->qs_db.update_status(this->sample_id, this->chip_type, this->chip_ver, this->open_array_id,
            this->status, val_assay_map.size(), inv_assay_set.size(), amb_assay_set.size()+faulty_cnt);
    assert(ret == true);
    ...
    for (set<string>::const_iterator it = this->lang_code_set.begin(); it != this->lang_code_set.end(); ++it)
    {
        ...
        time_t now;
        time(&now);
        this->gss_logger->log(true, " SAMPLE %s-%s | St:%c | %luV,%luI,%luA (%5.1f%%) | %s | %s | %lu secs\n",
        ...
        const string& _lang_code)
{
    ...
    string genetic_summary, comparative_risk, summary_title, genetic_summary_title, pop_zero_note, pat_zero_note, und_note, pat_legend, pop_legend, disclaimer, axis,
            risk_footnote1, risk_footnote2, risk_footnote3,
            table_risk_description, table_h_disease_condition, table_h_relative_genetic_risk, table_h_incidence, table_h_increased_risk,
            table_risk_caption, table_risk_text;
    ...
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "TITLE", "risk_table_col1", table_h_disease_condition);
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "TITLE", "risk_table_col2", table_h_relative_genetic_risk);
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "TITLE", "risk_table_col3", table_h_incidence);
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "TITLE", "risk_table_col4", table_h_increased_risk);
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "risk_perc", table_risk_caption);
    assert(ret == true);
    ret = trans_db.get_content(this->trans_conf.get_doc_id("label"), _lang_code.c_str(), this->trans_conf.get_version("label"), "LABEL", "relative_risk_interpretation", table_risk_text);
    assert(ret == true);
    ...
    xml_ret = gss_xml_writer.insert_summary(
            ...
            table_h_disease_condition.c_str(),
            table_h_relative_genetic_risk.c_str(),
            table_h_incidence.c_str(),
            table_h_increased_risk.c_str(),
            table_risk_caption.c_str(),
            table_risk_text.c_str());
    assert(xml_ret == true);
    ...
    while (dtect_sort.get_name(type, dis_drug_name, dis_drug_id) == true)
    {
        ...
        //VarDb var_db("VarDb", "192.168.7.1", "Manish", ""); //syhwah 2013-03-11
...
bool DtectMapper::generate_pdf(const string& _lang_code) const //syhwah 2013-03-15 :- added _lang_code
{
    ...
    if (latex_log_file != NULL)
        ...
        sprintf(pdf_gen_command, "%s %s %d >> %s 2>> %s",
                this->trans_conf.get_latex(), //syhwah 2013-03-25
                this->xml_output_filename.c_str(),
