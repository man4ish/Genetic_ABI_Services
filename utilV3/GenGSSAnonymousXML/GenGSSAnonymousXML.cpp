...
typedef struct _stGeneralInfo
{
    ...
    ~_stGeneralInfo()
    {
        ...
        if (pcPatientAge) delete[] pcPatientAge;
        ...
        if (pcCoverAssertion) delete[] pcCoverAssertion;
...
char g_acTmpOutFileName[4096], g_acQRCodeFileName[4096], g_acLangCode[3];
...
const int TAG_LANGUAGE_LEN = strlen(TAG_LANGUAGE);
...
const char *TAG_PATIENT_AGE = "    <PatientAge";
const int TAG_PATIENT_AGE_LEN = strlen(TAG_PATIENT_AGE);
...
const char *TAG_COVER_ASSERTION = "    <CoverAssertion";
const int TAG_COVER_ASSERTION_LEN = strlen(TAG_COVER_ASSERTION);
    ...
    while (!feof(g_pfIn)){
        ...
        if (!bGenInfo) { 
           ...
           {   
              ...
              if (strncmp(acbuf,TAG_LANGUAGE,TAG_LANGUAGE_LEN)==0)
              {
                 pChr = strchr(acbuf,'>')+1; pcTmp = strchr(pChr,'<'); *pcTmp=0;      
                 strcpy(g_acLangCode,pChr);
              } 
           }
        }
        ...
        if (strncmp(acbuf,TAG_GENERAL_INFO_END,TAG_GENERAL_INFO_END_LEN)==0)
        {
	     ...
	     fprintf(g_pfOut,"%s\n",g_stGeneralInfo.pcPatientAddress);
             ...
             if (g_stGeneralInfo.pcCoverAssertion) {
                 fprintf(g_pfOut,"%s\n",g_stGeneralInfo.pcCoverAssertion);
             }  
        ...
        else if (strncmp(acbuf,TAG_PATIENT_DOB,TAG_PATIENT_DOB_LEN)==0)
        {
             ...
             if (g_stGeneralInfo.cGender == 'F') {
                ...
                sprintf(g_stGeneralInfo.pcPatientDOB,"%s1963-01-02%s",pChr,pChr2);
             }
        }
        else if (strncmp(acbuf,TAG_PATIENT_AGE,TAG_PATIENT_AGE_LEN)==0)
        {
             int CurrentYear = (2000+g_ptm->tm_year)-100;

             pChr = acbuf; pcTmp = strchr(acbuf,'>')+1; pChr2 = strchr(pcTmp,'<'); *pcTmp=0;
             g_stGeneralInfo.pcPatientAge = new char[4096];

             if (g_stGeneralInfo.cGender == 'F') {                  
                sprintf(g_stGeneralInfo.pcPatientAge,"%s%d%s",pChr,(CurrentYear-1965),pChr2);
             } else {
                sprintf(g_stGeneralInfo.pcPatientAge,"%s%d%s",pChr,(CurrentYear-1963),pChr2);
             }
        }       
        ...
        else if (strncmp(acbuf,TAG_REFERENCE_NO,TAG_REFERENCE_NO_LEN)==0)
        {
             ...
             sprintf(g_acQRCodeFileName,"%s/%s_000%u_%s_AAA88_%s%02d%02d%02d.png",
                     g_pcOutFolder,g_pcChipType,g_usSampleId,g_pcCurrDate_fmt1,
                     g_pcCurrDate_fmt1,g_ptm->tm_hour,g_ptm->tm_min,g_ptm->tm_sec); 

             sprintf(g_stGeneralInfo.pcQrCode,"%s%s%s",pChr,g_acQRCodeFileName,pChr2);
        }
        ...
        else if (strncmp(acbuf,TAG_COVER_ASSERTION,TAG_COVER_ASSERTION_LEN)==0)
        {
             g_stGeneralInfo.pcCoverAssertion = new char[4096];

             strcpy(g_stGeneralInfo.pcCoverAssertion,acbuf);
        }       
    }
}
...
void GenerateQRCode()
{
     fprintf(stdout,"Generating QRCode...\n");

     char acQRCode[1024]; 
     sprintf(acQRCode,"%s/000%u/20%02d-%02d-%02d",g_pcChipType,g_usSampleId,g_ptm->tm_year-100,g_ptm->tm_mon+1,g_ptm->tm_mday);       

     char acCmdLine[1024];
     sprintf(acCmdLine,"qrencode -o %s -s 9 -m 1 \"%s\"",g_acQRCodeFileName,acQRCode); 

     int nRet = system(acCmdLine);

     if (nRet != 0) { fprintf(stdout,"Error code %d returned from qrencode.\n",nRet); }     
}


void RenameOutFileName()
{
    fprintf(stdout,"Rename Output File Name...\n");

    char acOutFileName[4096];
    int nLen = strlen(g_acQRCodeFileName); g_acQRCodeFileName[nLen-4]=0;     
    sprintf(acOutFileName,"%s_%s.xml",g_acQRCodeFileName, g_acLangCode);  

    char acCmdLine[4096];
    sprintf(acCmdLine,"mv %s %s",g_acTmpOutFileName,acOutFileName);

    int nRet = system(acCmdLine);

    if (nRet != 0) { 
       fprintf(stdout,"Error code %s returned from renaming output filename.\n",nRet); 
    } else {
...
int main(int argc, char *argv[])
{
    ...
    g_pfIn = fopen(argv[1],"r"); if (!g_pfIn) {fprintf(stdout,"Failed to open %s for reading. : %s\n",argv[1],strerror(errno)); goto ExitRtn;}  

    g_pcOutFolder = new char[strlen(argv[2])+1];
    strcpy(g_pcOutFolder, argv[2]);
 
    sprintf(g_acTmpOutFileName,"%s/Sample.xml",g_pcOutFolder);

    g_pfOut = fopen(g_acTmpOutFileName,"w"); 
    if (!g_pfOut) {fprintf(stdout,"Failed to open %s for writing. : %s\n",g_acTmpOutFileName,strerror(errno)); goto ExitRtn;}
    ...
    ProcessFile(); GenerateQRCode(); RenameOutFileName();
