...
<<echo=FALSE,results=tex>>=
	...
	cat("\\begin{tabular}{P{3.7cm}  P{6cm} }","\n")
  	...
  	cat(paste("\\textsf{",PatientAddressKey,":} & \\textsf{", padd[[1]][1] ,"}\\\\",sep = ""),"\n")
        ...
        #if(is.na(padd[[1]][5])== FALSE)
        #{
        #   if (padd[[1]][5] != "") {
        #      cat(paste(" & \\textsf{",padd[[1]][5],"}\\\\", sep = ""),"\n")
        #   }
        #}
...
#} else {
#    cat(paste("\\fancyhead[EL]{\\begin{mdframed}[linecolor=white,roundcorner=0pt,backgroundcolor=white,innerleftmargin=0,innertopmargin=0,innerbottommargin=0]\\parbox[t]{1.0\\textwidth}{\\raggedright","\\includegraphics[height=12mm]{",logo,"}}","\\parbox[t]{0.333\\textwidth}{\\centering Page}\\parbox[t]{0.333\\textwidth}{\\raggedleft 3} \\end{mdframed}}",sep=""),"\n")
#    cat(paste("\\fancyhead[OR]{\\begin{mdframed}[linecolor=white,roundcorner=0pt,backgroundcolor=white,innerleftmargin=0,innertopmargin=0,innerbottommargin=0]\\parbox[t]{1.0\\textwidth}{\\raggedleft", "\\includegraphics[height=12mm]{",logo,"}}","\\end{mdframed}}",sep=""),"\n")
#}
cat("\\definecolor{textcol}{rgb}{.118, .565, 1.00}","\n")
cat("\\fancyfoot[C]{\\begin{mdframed}[roundcorner=0pt,leftmargin=0, rightmargin=0, linecolor=white,outerlinewidth=.1, innerleftmargin=0,innertopmargin=0,innerbottommargin=0,innerrightmargin=0,everyline = true, splittopskip=.6cm, splitbottomskip=.3cm]","\n")
...
TermsContent1<-xmlValue(top[["Terms"]][["TermsContent1"]])
TermsContent2<-xmlValue(top[["Terms"]][["TermsContent2"]])
TermsContent3<-xmlValue(top[["Terms"]][["TermsContent3"]])
TermsContent4<-xmlValue(top[["Terms"]][["TermsContent4"]])
TermsContent5<-xmlValue(top[["Terms"]][["TermsContent5"]])
...
{
   if(lang == "zh") {
      cat(paste("\\includegraphics[trim=0.20cm 0.1cm 0cm 0cm, clip=true,width=",imagewidth_CRD,"\\linewidth]{",Section1,"}",sep=""),"\n")
   } else {
...
cat("~\\\\[-5pt]","\n")
     ...
     #Header5<-xmlValue(top[["Summary"]][["GeneticHeader"]][["page"]])
     ...
     SummaryNoRisk<-xmlValue(top[["Summary"]][["SummaryNoRisk"]]) 
     cat("\\vspace{-16pt}","\n")
     #cat("\\scriptsize ","\n")
...
if(runtype == 1)
{
   ...
   if(tag == "Disease" && as.numeric(data[5,rs]) > 1) {
     ...
     if (lang == "zh") {
         #cat("{\\renewcommand{\\arraystretch}{1.5}\\begin{longtable}{|M{2.2cm}|M{2.6cm}|M{2.2cm}|M{2.1cm}|M{0.5cm}|}","\n")
         cat("{\\renewcommand{\\arraystretch}{1.5}\\begin{longtable}{|M{2.4cm}|M{2.3cm}|M{2.2cm}|M{2.1cm}|M{0.8cm}|}","\n")
     } else {
     ...
     #cat(paste("\\rowcolor{textcol} \\textbf{ \\textcolor{white}{",Header1,"}} &\\textbf{ \\textcolor{white}{",Header2,"}} &\\textbf{ \\textcolor{white}{",Header3,"}}  &\\textbf{ \\textcolor{white}{",Header4,"}}  \\\\",sep=""),"\n")
     cat("\\endhead","\n")
...
if(nrow(sorteddata) != 0 )
{
  for (rs in 1:nrow(sorteddata)) {
                 #print(indexmap[[sorteddata[rs,1]]])
                 cat("\\rowcolor{rowcol}","\n")
                 #cat(paste(indexmap[[sorteddata[rs,1]]],sep=""),"\n")
...
else
{
   #if(rcount == 0)
   {
        ...
        #cat(paste("\\multicolumn{4}{|c|}{m{4cm}}{ ",text,"} \\\\",sep=""),"\n")
        cat("\\hline","\n")
   }
}
      ...
      if(length(riskfile[[disid]]) !=0)
      {
        ...
        cat(paste("maxval = ",maxval,sep=""))
         	...
         	#{ 
         		...
         		else if(riskvalue < 1.3) #else if(riskvalue > 1.0001 && riskvalue < 1.3)
         		{
...
{       
     ...
     #cat("\\hline","\n")     
     if (lang=="zh"){
         cat("{\\renewcommand{\\arraystretch}{1.5}\\begin{longtable}{|M{1.4cm}|M{1.4cm}|M{1.1cm}|M{1.2cm}|M{2.4cm}|M{1.6cm}|}","\n") 
     } else {
	...
	title<-xmlValue(diseases[i][["Disease"]][["References"]][["Title"]])
        ...
        if(rc !=0 )
        {
           if (lang != "zh") {
               ...
               if (ChipType == "ONC"){   
                   if (disid=="ONC_01_06"||disid=="ONC_01_11"||disid=="ONC_01_33") { 
                       cat("~\\\\[-5pt]","\n")                    
                   } else if(disid=="ONC_01_21") {
                       cat("~\\\\[7pt]","\n")                       
                   } 
                   else if(disid=="ONC_01_20"||disid=="ONC_01_27") {
                       cat("\\newpage","\n") 
                   } else { 
                       cat("~\\\\","\n")
                       cat("\\needspace{1cm}","\n")
    ...
    if(lang == "en") {
        ...
        {   
        	#if(risk != TypicalRisk)
                if(risk != TypicalRisk || mayocontentflag == 1) #syhwah 
          	{
       ...
       if(risk != TypicalRisk || mayocontentflag == 1) #syhwah
       {
        	...
        	{             
           		...
           		booltab<-grep("<table", xmlValue(Symptoms[["Text"]]))             
                        ...
                        if(lang == "en") { cat("\\needspace{1cm}","\n") } 
                              ...
                              cat("\\\\","\n")
                              if (ChipType == "CRD") {  
                                  if (disid != "CRD_01_11" && disid != "CRD_01_12"  && 
                                      disid != "CRD_01_13" && disid != "CRD_01_08") 
                                  {
                                      cat("\\\\","\n") 
                                  }
                              }
        ...
        if(risk == IncreasedRisk || mayocontentflag == 1) #syhwah 
        {
        	if(length(Risk[["Title"]])!=0)
        	{
           		if(lang == "en") { cat("\\needspace{1cm}","\n") }
                        else {
                              if (disid == "ONC_01_24") { cat("\\newpage","\n") }
                              else {     
                                  cat("~\\\\","\n")
                                  cat("\\\\","\n")
                              }  
                              ...
                              if (ChipType == "CRD") {     
                                  if (disid != "CRD_01_11" && disid != "CRD_01_09" && disid != "CRD_01_05" && 
                                      disid != "CRD_01_13" && disid != "CRD_01_07") { 
                                      cat("\\\\","\n") 
                                  }
        	...
        	{         
           		if(lang == "en") { cat("\\needspace{1.0cm}","\n") }
                              ...
                              cat("~\\\\","\n")
                              if (ChipType == "CRD") {  
                                  if (disid != "CRD_01_09" ) { cat("\\\\","\n") }
                                  if (disid == "CRD_01_13") { cat("\\\\","\n") }  
                                  if (disid == "CRD_01_07") { 
                                      cat("\\\\","\n") 
                                      cat("\\\\","\n")
                                  } 
        ...
        if(risk != TypicalRisk || mayocontentflag == 1) #syhwah
        {
        	if(length(Lifestyle[["Title"]])!=0)
        	{
           		if(lang == "en") { cat("\\needspace{1cm}","\n") }
                              ...
                              cat("\\\\","\n")
                              if (ChipType == "CRD") { 
                                  if (disid == "CRD_01_01" || disid == "CRD_01_09") { cat("\\\\","\n") }
                              }
                        }
